...
//const data = @embedFile("03.test-input2");
...
pub fn main() !void {
    ...
    var rem = try RegEx.compile(allocator, "do\\(\\)|don't\\(\\)|mul\\((\\d{1,3}),(\\d{1,3})\\)");
    var sum: i64 = 0;
    while (splits.next()) |line| {
        ...
        while (capx != null) {
            const cap = capx.?;
            const tx = (cap.sliceAt(0)).?;
            const sl = (cap.boundsAt(0)).?;
            pos += sl.upper;
            std.debug.print("'{s}'\n", .{tx});
            if (std.mem.eql(u8, tx, "do()")) {
                enabled = true;
            }
            else if (std.mem.eql(u8, tx, "don't()")) {
                enabled = false;
            }
            else if (enabled){
